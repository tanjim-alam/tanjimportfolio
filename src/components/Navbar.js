...
export default function Navbar() {
  ...
  return (
    <div className="fixed top-5 left-0 right-0 z-50 flex flex-col items-center px-4 gap-2">
      ...
      {/* ── Pill bar — always rounded-full, never changes shape ── */}
      <nav
        className={`w-full max-w-5xl px-8 rounded-full transition-all duration-300 ${
          scrolled
            ...
            {navLinks.map(({ href, label }) => (
              <a
                ...
                className={`px-3 py-1.5 text-[14px] font-medium transition-all duration-200 rounded-full ${
                  active === href ? 'text-white' : 'text-white hover:text-white/90'
                }`}
                style={
                  active === href
      ...
      {/* ── Mobile menu — separate box below the pill ── */}
      <div
        className={`md:hidden w-full max-w-5xl rounded-2xl overflow-hidden transition-all duration-300 ${
          menuOpen
          ...
          {navLinks.map(({ href, label }) => (
            <a
              ...
              className={`text-sm py-2.5 px-3 rounded-xl transition-all duration-200 ${
                active === href ? 'text-white' : 'text-white/90 hover:text-white/90'
              }`}
              style={
                active === href
